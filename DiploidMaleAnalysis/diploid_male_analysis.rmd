...
```{r load consensus genotyping matrix, include = FALSE}
finalmaletypes <- read.delim("Consensus_male_genotypes.txt")
```
...
```{r diploid male plot, include = FALSE}
#plot stacked bar chart of haploid vs diploid males in each colony, and grid graphs by colony type
ggplot(finalmaletypes, aes(x = Colony)) +
  geom_bar(aes(colour = Ploidy2)) +
  facet_wrap(vars(Type))
```

This doesn't quite give me what I want, since the Colony column values are treated as integers, and so the x axis is treated as continuous. Convert colony number to character and plot again.

```{r convert colony numbers and plot diploid male graph, include = FALSE}
#convert Colony to character
finalmaletypes <- finalmaletypes %>% convert(chr(Colony))

#plot diploid male bar chart
...
This isn't very pretty, plot and edit using Excel and Illustrator for plot in manuscript.
